...
$.extend($.ui.rlightbox, {
	global: {
		...
		getCurrentElementNumber: function( element, url, options ) {
			var _currentNumber,
				data = this.data,
				sets = this.sets,
				self = this,
			...
			$.each(_currentSet, function(i, v) {
				...
				// compare elements URL
				if (options.preventSameUrl){
					if ( url == v.url ) {
						_currentNumber = i + 1;
				...
				} else if ( $(element).get(0) === v.element.get(0) ) {
					_currentNumber = i + 1;
		...
		open: function( thisElement ) {
			...
			// did not find another way to get options in getCurrentElementNumber() instead of passing as argument
			data.currentElementNumber = this.getCurrentElementNumber( _jqElement, _currentUrl, thisElement.options );
